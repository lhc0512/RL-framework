...
if __name__ == '__main__':
    ...
    while current_step <= args.total_step:
        ...
        while not done:
            ...
            if current_step % args.train_step == 0:
                agent.train()
            if current_step % args.test_step == 0:
                average_episode_reward = test_agent(test_env, agent)
                episode_reward_history.append(average_episode_reward)
                print(f'| step : {current_step:6} | Episode Reward: {average_episode_reward:5.1f} |')
            # if current_step % args.save_step == 0:
            #     agent.save(f'{args.checkpoint_path}/{args.seed}-{args.name}')
